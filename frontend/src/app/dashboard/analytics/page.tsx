...
export default function AnalyticsPage() {
  ...
  // Build monthly expense data matching the same months
  ...
  // Enrich monthlySales with expenses + profit
  ...
  const tabBtn = (t: typeof tab, label: string) => (
    <button onClick={() => setTab(t)} style={{
      ...
      background: tab === t ? 'linear-gradient(135deg,#7c3aed,#3b82f6)' : theme.hover,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      {tab === 'revenue' && enrichedMonthly.length > 0 && (
        <div style={{ ...card, padding: '20px' }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: theme.text, marginBottom: 16 }}>Monthly Revenue (Last 6 Months)</div>
          <ResponsiveContainer width="100%" height={280}>
            <ComposedChart data={enrichedMonthly}>
              ...
              <Bar dataKey="orders" fill="#3b82f633" name="Orders" yAxisId={0} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {tab === 'profit' && (
        <div style={{ ...card, padding: '20px' }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: theme.text, marginBottom: 4 }}>Revenue vs Expenses vs Profit</div>
          <div style={{ fontSize: 12, color: theme.textFaint, marginBottom: 16 }}>Monthly comparison — expenses are calculated from your expense records</div>
          {enrichedMonthly.length > 0 ? (
            ...
            <div style={{ padding: 40, textAlign: 'center', color: theme.textFaint }}>No monthly data yet — create some orders and expenses.</div>
          )}
        </div>
      )}
      ...
      {tab === 'categories' && (
        <div style={{ display: 'grid', gridTemplateColumns: categoryData.length ? '1fr 1fr' : '1fr', gap: 16 }}>
          {categoryData.length > 0 ? (
            <>
              <div style={{ ...card, padding: '20px' }}>
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie data={categoryData} dataKey="revenue" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={({ name, percent }) => `${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>
            ...
            <div style={{ ...card, padding: 40, textAlign: 'center', color: theme.textFaint }}>No category revenue data yet.</div>
          )}

          {/* Payment methods pie */}
          {methodData.length > 0 && (
            <div style={{ ...card, padding: '20px', gridColumn: categoryData.length ? '1 / -1' : '1' }}>
