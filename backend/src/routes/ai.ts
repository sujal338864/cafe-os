...
- Top selling products: ${topProducts.map(p => `${p.name} (₹${p._sum.total})`).join(', ')}
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `You are an AI business assistant for a shop management system. Use the context below to answer questions accurately and concisely. Always be specific with numbers and provide actionable insights. Context: ${context}`
          },
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const outstandingPayments = await prisma.customer.aggregate({
      where: { shopId, outstandingBalance: { gt: 0 } },
      _sum: { outstandingBalance: true },
      _count: true
    });

    if (outstandingPayments._count > 0) {
      insights.push({
        type: 'info',
        icon: '💳',
        title: 'Outstanding Payments',
        message: `${outstandingPayments._count} customers owe ₹${Number(outstandingPayments._sum.outstandingBalance || 0).toFixed(2)}. Send reminders.`,
    ...
    if (topProducts.length > 0) {
      insights.push({
        ...
        icon: '⭐',
        title: 'Top Product',
        message: `${topProducts[0].name} is your best seller this week. Consider increasing stock.`,
        action: 'View Products'
      });
    }
